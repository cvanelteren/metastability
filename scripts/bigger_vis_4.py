...
path = Path("./")
fp = path.joinpath("combined_errors.pkl")
...
# e = np.stack(errors["success_1_to_0"])
# e = e[:, 0] / e[:, 1]
# errors["success"] = e
...
l = "id label seed nudge success_0_to_1 success_1_to_0".split()

other = pd.read_pickle("./success_n.pkl")

try:
    errors = errors.drop(labels="success_0_to_1 success_1_to_0".split())
except:
    print()
# errors = pd.merge(errors, other, left_on=l, right_on=l)
...
fig.show()
fig.savefig("./figures/fig4_ER_all_alt.png")
print("showing")
plt.show(block=1)
