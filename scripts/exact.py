...
"""
To Future-Me I need to clean this file up.
"""
...
class SystemToSystem(AbstractSimulator):
    def setup(self, allowed=[]):
        self.allowed = allowed
        n = self.states.shape[1]  # should be 2**n x n
        # P(S^t : s_i)
        self.evolve_states = np.eye(2**n)
        self.evole_states = self.p_state.copy()
        self._H = entropy(self.p_state)
    ...
    def update(self):
        pst = self.transfer_matrix @ self.evolve_states
        # pst = self.evolve_states @ self.transfer_matrix
        HC = self.p_state @ entropy(pst)
        self.evolve()
        return self._H, HC, self._H - HC, pst, self.evolve_states
...
def ising(states, A, node=None, eta=None, gamma=0):
    from itertools import combinations

    # normal ising, without external magnetic field
    H = -np.einsum("ij, ij->ij", (states * 2 - 1), (states * 2 - 1).dot(A))
    # higher order interactions according to Rosas
    if gamma >= 1:
        assert type(gamma) is int
        N = np.arange(0, len(A))
        for c in combinations(N, gamma):
            H -= np.prod((states * 2 - 1)[:, c], axis=-1).sum(0)
    return H
...
def run(
    ...
    from exact_utils import match_temp_stc
    from exact import NodeToSystem, NodeToMacroBit, SystemToNode
...
def get_conditional(p, states, x, y):
    """
    Returns p(y | x) for a given pair x,y and joint distribution p
    """
    # return p(y | x)
    p_joint = {}
    p_conditional = {}
    ...
    for idx, si in enumerate(states):
        s = (si[x], si[y])
        p_joint[s] = p_joint.get(s, 0) + p[idx]
        if si[x] not in p_conditional:
            p_conditional[si[x]] = {}
        p_conditional[si[x]][si[y]] = p_conditional[si[x]].get(si[y], 0) + p[idx]

        p_marginal[si[y]] = p_marginal.get(si[y], 0) + p[idx]
    for k, v in p_conditional.items():
        z = sum(v.values())
        for kk, vv in v.items():
            p_conditional[k][kk] /= z
    return p_joint, p_conditional, p_marginal


def get_pairwise(p, states):
    """
    Make pairwise MI
    """
    n = states.shape[1]
    ...
    for x in range(n):
        ...
        for y in range(n):
            # if x < y:
            pj, pc, pm = get_conditional(p, states, x, y)
            H = 0
            Hc = 0
            for k, v in pc.items():
                H -= pm[k] * np.log2(pm[k])
                for idx, (kk, vv) in enumerate(v.items()):
                    Hc -= pm[k] * vv * np.log2(vv)
            I[x, y] = 1 - Hc / H
    return I
